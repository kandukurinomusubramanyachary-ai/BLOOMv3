...
export default function SessionControls({ paused, muted, onPause, onMute, onStop, voiceAvailable = true, disabled = false, TourTarget }) {
  ...
  return <View style={s.row} accessibilityLabel="Session controls">
    ...
    <Target id="strength-pause" style={s.control}><StrengthButton title={paused ? 'Resume' : 'Pause'} icon={paused ? 'play-outline' : 'pause-outline'} onPress={onPause} disabled={disabled} style={s.control} /></Target>
